...
jQuery(document).ready(function($) {
    ...
    function formatMessage(message) {
        try {
            ...
            for (let i = 0; i < sections.length; i++) {
                if (i % 2 === 0) {
                    rawHtml += marked.parse(sections[i]);
                } else {
                    ...
                    const escapedCodeBody = $('<div>').text(codeBody).html();
                    rawHtml += `<pre><code class="language-${language}">${escapedCodeBody}</code></pre>`;
                }
            }
            // Sanitize the HTML generated by marked
            const cleanHtml = DOMPurify.sanitize(rawHtml);
            // Make links clickable and add security attributes AFTER sanitization
            let finalHtml = cleanHtml.replace(/<a\s+href="([^"]+)"([^>]*)>/g,
                '<a href="$1" target="_blank" rel="noopener noreferrer"$2>');
            return finalHtml;
        } catch (e) {
            console.error('Error parsing markdown:', e);
            return $('<p>').text(message).html();
        }
    }
    ...
    function addMessage(message, isUser) {
        ...
        if (isUser) {
            ...
            // Bot messages are sanitized and formatted
    ...
    async function addMessageWithAnimation(message, isUser) {
        ...
        const sections = message.split(/```([\s\S]*?)```/g);
        let currentContentForAnimation = '';
        for (let i = 0; i < sections.length; i++) {
            ...
            if (i % 2 === 0) {
                ...
                for (let j = 0; j < words.length; j += chunkSize) {
                    ...
                    currentContentForAnimation += chunk;
                    messageDiv.html(formatMessage(currentContentForAnimation));
                    messagesContainer.scrollTop(messagesContainer[0].scrollHeight);
                    await new Promise(resolve => setTimeout(resolve, 50));
                }
            } else {
                currentContentForAnimation += '```' + section + '```';
                messageDiv.html(formatMessage(currentContentForAnimation));
                messagesContainer.scrollTop(messagesContainer[0].scrollHeight);
                await new Promise(resolve => setTimeout(resolve, 200));
            }
        }
        // Ensure the final full message is also sanitized
        messageDiv.html(formatMessage(message));
        messagesContainer.scrollTop(messagesContainer[0].scrollHeight);
        isAnimating = false;
    }
